...
- **採用**
  - フルスタックエンジニア、バックエンドエンジニア、MLOpsエンジニア、データエンジニア、機械学習エンジニアのポジションの採用要件・戦略を担当。採用プラットフォームやエージェント（5社）、直接のヘッドハンティング、リファラルなどを活用し、5名以上のエンジニアの採用を行いまいした。
- **目標設定・管理**
...
- **開発風土・制度設計**
  ...
  - フルリモートおよび英語公用語の組織における連携不足を補うため、定期的に対面での開発やブレインストーミングの場を設ける制度を導入しました。結果、プロダクトの開発品質とスピードが向上し、多くのエンジニアが継続を希望する組織風土を形成しました。
    - AI時代における開発体制に移行するため、経費制度を作成し、組織のほぼ全員がcodex / cursor・claude code等の最新のAIエージェントを複数使える体制を整えました。
- **MLOpsチームのマネジメント**
  - チーム設立時から参画し、定例ミーティング、開発ルール、チームバックログの整備、メンバーアサインおよび進捗管理を主導しました。
  - 成果を評価され、3ヶ月でチームリーダー、1年でチームマネージャーに昇進。チーム規模を3名から最大6名に拡大しました
- **開発サポートチームのマネジメント**
  - QA・雑務・自動化しにくいトイルを定義し、新メンバーの採用を行い、エンジニアが開発に集中できる体制を整えました。
- **プロダクト開発チームの立て直し**
...
- **AI同時通訳サービスの開発**
  ...
  - 企業の機密情報保護ニーズに対応したInPrivateモード（通訳履歴非保存）やパスコード制御機能を実装
  - OIDC/OAuth2準拠の認証基盤をスクラッチで設計・実装。アクセストークン・リフレッシュトークン・IDトークンの発行・ローテーション・検証の仕組みを構築し、エンタープライズ顧客向けの認証統合を実現
  - OpenAPI定義に基づいて全APIエンドポイントの操作を自動記録する監査ログ基盤を設計・実装。日英多言語対応を含め、エンタープライズ顧客の監査要件に対応
...
- **セキュリティの向上**
  ...
  -  IT資産全般の管理を、MDMやマルウェア対策ソフトの導入を通して、推進しました。
- **AWSインフラコスト管理**
  - 社内のインフラコストを管理していました。経営層に向けたコストの報告、タグづけによるチーム境界整備、定期的なコスト削減のための変更・ハッカソン開催・AIを用いた自動コスト分析、意図しない課金を防ぐためのSlack通知Bot整備などを行いました
...
- **ウェアハウス層のモデリング**
  - ステージング層を導入し、一部でディメンショナルモデリングやワイドテーブルを準備し、dbtで全て管理する体制を整えました
- **社内データ分析文化の醸成**
